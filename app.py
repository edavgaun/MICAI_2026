...
st.markdown("""
    <style>
        .block-container { padding-left: 2rem !important; padding-right: 2rem !important; padding-top: 3.5rem !important; }
        div.stButton > button { height: 45px !important; padding: 0px 25px !important; }
    </style>
...
with nav_col1:
    if st.button("🏠 Home Overview", key="btn_home"): st.session_state["active_tab"] = "Home"
with nav_col2:
    if st.button("🕸️ Network Discovery", key="btn_network"): st.session_state["active_tab"] = "Network"
...
if st.session_state["active_tab"] == "Home":
    ...
    # 🕒 LÍNEA DE TIEMPO POR AÑO (Reemplaza los botones de Eras)
    ...
    # Mostramos dinámicamente la Era correspondiente
    st.subheader(f"📖 {text.obtener_info_era(selected_year)}")

    filter_col1, filter_col2 = st.columns(2)
    with filter_col1:
        dropdown_area = st.selectbox("Área de Investigación:", ['All Areas'] + areas)
    with filter_col2:
        selected_institutions = st.multiselect("🔍 Resaltar Entidades:", options=sorted(df_final['institution_clean'].unique()))
